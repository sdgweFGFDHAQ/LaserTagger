...
def get_position_encoding(
        length, hidden_size, min_timescale=1.0, max_timescale=1.0e2):  # TODO previous max_timescale=1.0e4
...
def get_decoder_self_attention_bias(length, dtype=tf.float32):
    ...
    with tf.name_scope("decoder_self_attention_bias"):
        valid_locs = tf.linalg.band_part(input=tf.ones([length, length], dtype=dtype),  # 2个length分别是输入和输出的最大长度
                                         num_lower=-1, num_upper=0)  # Lower triangular part is 1, other is 0
